...
#define IPv4_TX_API_LENGTH 10
#define SMS_TX_API_LENGTH 21
...
#define IPv4_TX_REQUEST 0x20
#define IPv4_RX_RESPONSE 0xB0
#define SMS_TX_REQUEST 0x1F
#define SMS_RX_RESPONSE 0x9F
...
/**
 * IPv4 TX STATUS constants
 */
#define SUCCESSFUL_TRANSMIT 0x0
#define FAILURE_TRANSMIT_CELL_NETWORK 0x21
#define NOT_REGISTERED_CELL_NETWORK 0x22
#define	INVALID_FRAME_VALUES 0x2C
#define INTERNAL_ERROR 0x31
#define	RESOURCE_ERROR  0x32
#define MESSAGE_TOO_LONG 0x74
#define SOCKET_CLOSED_UNEXPECTEDLY 0x76
#define INVALID_UDP_PORT 0x78
#define INVALID_TCP_PORT 0x79
#define INVALID_HOST_ADDRESS 0x7A
#define INVALID_DATA_MODE 0x7B
#define INVALID_INTERFACE 0x7C
#define INTERFACE_NOT_ACCEPTING_FRAMES 0x7D
#define CONNECTION_REFUSED 0x80
#define SOCKET_CONNECTION_LOST 0x81
#define NO_SERVER 0x82
#define SOCKET_CLOSED 0x83
#define UNKNOWN_SERVER 0x84
#define UNKNOWN_ERROR 0x85
#define INVALID_TLS_CONFIGURATION 0x86
...
#define IPv4_DEFAULT_SOURCE_PORT 0
#define IPv4_TCP_PROTOCOL 1
#define IPv4_LEAVE_OPEN 0

#define SMS_DEFAULT_OPTIONS 0
/**
...
public:
	...
	void getRxIPv4Response(XBeeResponse &response);
	
	void getRxSMSResponse(XBeeResponse &response);
...
class CellTxStatusResponse : public FrameIdResponse {
	public:
	CellTxStatusResponse();
	uint8_t getStatus();
	bool isSuccess();
...
class IPv4RxResponse : public RxDataResponse {
	public:
	IPv4RxResponse();
	uint32_t getSourceAddrIPv4();
	...
	static const uint8_t API_ID = IPv4_RX_RESPONSE;
	private:
	uint32_t _sourceAddrIPv4;
};

class SMSRxResponse : public RxDataResponse {
	public:
	SMSRxResponse();
	uint8_t* getPhoneNumber();
	uint16_t getPhoneNumberLength();
	...
	static const uint8_t API_ID = SMS_RX_RESPONSE;
	private:
	uint16_t _phoneNumberLength;
};
...
public:
	 ...
	 */
	void setPayload(uint8_t* payloadPtr, uint16_t payloadLength) {
...
class IPv4TxRequest : public PayloadRequest {
public:
	/**
	 * Creates a TX Request message causes the device to transmit data in IPv4 format with DEFAULT_FRAME_ID
	 */
	IPv4TxRequest(uint32_t destAddrIPv4, uint16_t destPort, uint8_t *payload, uint16_t payloadLength);
	IPv4TxRequest(uint32_t destAddrIPv4, uint16_t destPort, uint16_t sourcePort, uint8_t protocol, uint8_t options, uint8_t *payload, uint16_t payloadLength, uint8_t frameId);
	/**
	 * Creates a default instance of this class.
	 */
	uint32_t getDestAddrIPv4();
	uint16_t getDestPort();
	uint16_t getsourcePort();	
	uint8_t getProtocol();
	uint8_t getOptions();
	void setDestAddrIPv4(uint32_t destAddrIPv4);
...
class SMSTxRequest : public PayloadRequest {
public:
	/**
	 * Creates a TX Request message causes the device to transmit data in IPv4 format with DEFAULT_FRAME_ID
	 */
	SMSTxRequest(uint8_t *phoneNumber, uint8_t *payload, uint16_t payloadLength);
	SMSTxRequest(uint8_t options, uint8_t *phoneNumber, uint8_t *payload, uint16_t payloadLength, uint8_t frameId);
	...
	void setOptions(uint8_t options);
	void setPhoneNumber(uint8_t* phoneNumberPtr);
protected:
	// declare virtual functions
	uint8_t getFrameData(uint16_t pos);
	uint16_t getFrameDataLength();
	
	uint8_t* _phoneNumberPtr;
	uint8_t _options;
};
...
public:
	 ...
	 */
	ZBTxRequest(const XBeeAddress64 &addr64, uint8_t *payload, uint16_t payloadLength);
	ZBTxRequest(const XBeeAddress64 &addr64, uint16_t addr16, uint8_t broadcastRadius, uint8_t option, uint8_t *payload, uint16_t payloadLength, uint8_t frameId);
	/**
	 * Creates a default instance of this class.  At a minimum you must specify
...
public:
	 ...
	 * normal ZBTxRequest.
	 */
	ZBExplicitTxRequest(XBeeAddress64 &addr64, uint8_t *payload, uint16_t payloadLength);
	/**
	 * Create a ZBExplicitTxRequest, specifying all fields.
	 */
	ZBExplicitTxRequest(XBeeAddress64 &addr64, uint16_t addr16, uint8_t broadcastRadius, uint8_t option, uint8_t *payload, uint16_t payloadLength, uint8_t frameId, uint8_t srcEndpoint, uint8_t dstEndpoint, uint16_t clusterId, uint16_t profileId);
	/**
...
public:
	...
	AtCommandRequest(uint8_t *command, uint8_t *commandValue, uint16_t commandValueLength);
	uint8_t getFrameData(uint16_t pos);
	uint16_t getFrameDataLength();
	uint8_t* getCommand();
	...
	uint16_t getCommandValueLength();
	void setCommandValueLength(uint16_t length);
	/**
...
public:
	 ...
	 */
	RemoteAtCommandRequest(uint16_t remoteAddress16, uint8_t *command, uint8_t *commandValue, uint16_t commandValueLength);
	/**
	 ...
	 */
	RemoteAtCommandRequest(XBeeAddress64 &remoteAddress64, uint8_t *command, uint8_t *commandValue, uint16_t commandValueLength);
	/**
	 * Creates a RemoteAtCommandRequest with 16-bit address to query a command.
	 * 16-bit address defaults to broadcast and applyChanges is true.
	 */

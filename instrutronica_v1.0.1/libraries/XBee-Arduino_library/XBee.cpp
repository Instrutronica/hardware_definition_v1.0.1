 ...
 * Ultima vez modificado 22/09/2019
 */
...
/*********************************************************agregado por david 15/sep/2019***************************************************/
CellTxStatusResponse::CellTxStatusResponse() : FrameIdResponse() {

}

uint8_t CellTxStatusResponse::getStatus() {
	return getFrameData()[1];
}

bool CellTxStatusResponse::isSuccess() {
	return getStatus() == SUCCESS;
}

IPv4RxResponse::IPv4RxResponse(): RxDataResponse() {
}

uint16_t IPv4RxResponse::getDestPort() {
	return 	(getFrameData()[4] << 8) + getFrameData()[5];
}

uint16_t IPv4RxResponse::getSourcePort() {
	return 	(getFrameData()[6] << 8) + getFrameData()[7];
}

uint8_t IPv4RxResponse::getProtocol() {
	return getFrameData()[8];
}

uint8_t IPv4RxResponse::getDataOffset() {
	return 10;
}

uint16_t IPv4RxResponse::getDataLength() {
	return getPacketLength() - getDataOffset() - 1;
}

uint32_t IPv4RxResponse::getSourceAddrIPv4() {
	return (uint32_t(getFrameData()[0]) << 24) + (uint32_t(getFrameData()[1]) << 16) + (uint16_t(getFrameData()[2]) << 8) + getFrameData()[3];
}

void XBeeResponse::getRxIPv4Response(XBeeResponse &rxIPv4Response) {

	IPv4RxResponse* IPV4rx = static_cast<IPv4RxResponse*>(&rxIPv4Response);

	IPV4rx->setFrameData(getFrameData());
	setCommon(rxIPv4Response);
}

SMSRxResponse::SMSRxResponse(): RxDataResponse() {	
}

uint8_t* SMSRxResponse::getPhoneNumber() {
	if (getPhoneNumberLength() > 0) {
		return getFrameData() + 1;
	}
...
uint16_t SMSRxResponse::getPhoneNumberLength() {
	_phoneNumberLength = 0;
	for (int i=0; i < 20; i++) {
		if (getFrameData()[i+1] != 0x00) {
			_phoneNumberLength++;
		}
	}
	return _phoneNumberLength;
}
uint8_t SMSRxResponse::getDataOffset() {
	return 20;
}

uint16_t SMSRxResponse::getDataLength() {
	return getPacketLength() - getDataOffset() - 1;
}

void XBeeResponse::getRxSMSResponse(XBeeResponse &rxSMSResponse) {
	SMSRxResponse* SMSrx = static_cast<SMSRxResponse*>(&rxSMSResponse);

	SMSrx->setFrameData(getFrameData());
	setCommon(rxSMSResponse);
}
...
uint16_t XBeeResponse::getPacketLength() {
	return ((uint16_t(_msbLength) << 8) & 0xff00) + (_lsbLength & 0xff);
}
...
bool XBee::readPacket(unsigned long timeout) {
...
void XBee::readPacket() {
    ...
    while (available()) {
		...
		if (available() > 200 && stopSendData == false) {
			digitalWrite(43, HIGH);
			stopSendData = true;
		}
		if (available() < 3 && stopSendData == true) {
			digitalWrite(43, LOW);
			stopSendData = false;
		}
...
/******************************************************************agregado por david 15/sep/2019***********************************************************/
IPv4TxRequest::IPv4TxRequest(uint32_t destAddrIPv4, uint16_t destPort, uint16_t sourcePort, uint8_t protocol, uint8_t options, uint8_t *data, uint16_t dataLength, uint8_t frameId): PayloadRequest(IPv4_TX_REQUEST, frameId, data, dataLength){
	_destAddrIPv4 = destAddrIPv4;
	_destPort = destPort;
...
IPv4TxRequest::IPv4TxRequest(uint32_t destAddrIPv4, uint16_t destPort, uint8_t *data, uint16_t dataLength): PayloadRequest(IPv4_TX_REQUEST, DEFAULT_FRAME_ID, data, dataLength) {
	_destAddrIPv4 = destAddrIPv4;
	_destPort = destPort;
	_sourcePort = IPv4_DEFAULT_SOURCE_PORT;
	_protocol = IPv4_TCP_PROTOCOL;
	_options = IPv4_LEAVE_OPEN;
}	
	
uint8_t IPv4TxRequest::getFrameData(uint16_t pos) {
	if (pos == 0) {
		return (_destAddrIPv4 >> 24) & 0xff;
		} else if (pos == 1) {
		return (_destAddrIPv4 >> 16) & 0xff;
		} else if (pos == 2) {
		return (_destAddrIPv4 >> 8) & 0xff;
		} else if (pos == 3) {
		return _destAddrIPv4 & 0xff;
		
		} else if (pos == 4) {
		return (_destPort >> 8) & 0xff;
		} else if (pos == 5) {
		return _destPort & 0xff;
		...
		return getPayload()[pos - IPv4_TX_API_LENGTH];
	}
}	

uint16_t IPv4TxRequest::getFrameDataLength() {
	return IPv4_TX_API_LENGTH + getPayloadLength();
}	

uint32_t IPv4TxRequest::getDestAddrIPv4() {
	return _destAddrIPv4;
}

uint16_t IPv4TxRequest::getDestPort() {
	return _destPort;
}

uint16_t IPv4TxRequest::getsourcePort() {
	return _sourcePort;
}

uint8_t IPv4TxRequest::getProtocol() {
	return _protocol;
}

uint8_t IPv4TxRequest::getOptions() {
	return _options;
}

void IPv4TxRequest::setDestAddrIPv4(uint32_t destAddrIPv4) {
	_destAddrIPv4 = destAddrIPv4;
}

void IPv4TxRequest::setDestPort(uint16_t destPort) {
	_destPort = destPort;
}

void IPv4TxRequest::setsourcePort(uint16_t sourcePort) {
	_sourcePort = sourcePort;
}

void IPv4TxRequest::setProtocol(uint8_t protocol) {
	_protocol = protocol;
}

void IPv4TxRequest::setOptions(uint8_t options) {
	_options = options;
}


SMSTxRequest::SMSTxRequest(uint8_t options, uint8_t *phoneNumber, uint8_t *data, uint16_t dataLength, uint8_t frameId): PayloadRequest(SMS_TX_REQUEST, frameId, data, dataLength){
	_options = options;
	_phoneNumberPtr = phoneNumber;
}

SMSTxRequest::SMSTxRequest(uint8_t *phoneNumber, uint8_t *data, uint16_t dataLength): PayloadRequest(SMS_TX_REQUEST, DEFAULT_FRAME_ID, data, dataLength) {
	_options = SMS_DEFAULT_OPTIONS;
	_phoneNumberPtr = phoneNumber;
}

uint8_t SMSTxRequest::getFrameData(uint16_t pos) {
	if (pos == 0) {
		return _options;
		} else if (pos > 0 && pos < 21) {
		return getPhoneNumber()[pos - 1];
		} else {		
		return getPayload()[pos - SMS_TX_API_LENGTH];
	}
}

uint16_t SMSTxRequest::getFrameDataLength() {
	return SMS_TX_API_LENGTH + getPayloadLength();
}

uint8_t SMSTxRequest::getOptions() {
	return _options;
}

uint8_t* SMSTxRequest::getPhoneNumber(){
	return _phoneNumberPtr;
}

void SMSTxRequest::setOptions(uint8_t options) {
	_options = options;
}

void SMSTxRequest::setPhoneNumber(uint8_t* phoneNumberPtr) {
